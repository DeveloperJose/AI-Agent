...
public class HankScorpio extends Player {
    protected final String newName = "HankScorpio"; // Overwrite this variable
                                                    // in your player subclass
    ...
    public Action makeAction() {
        ...
        if (graph[currentNode].getNeighbor(maxIndex).getPossibleCards().size() == 0) {
            Random r = new Random();
            // int neighbor =
            // graph[currentNode].getNeighbor(maxIndex).getNodeID();
            int neighbor = graph[currentNode].getNeighbor(r.nextInt(graph[currentNode].getNeighborAmount()))
                    .getNodeID();
