 ...
 * Note you can change the parameter settings and the number of games by
 * altering numGames and parameterSetting
 * 
...
public class GameMaster {
    ...
    private static int numGames = 2; // use a small number for quick tests, a
            ...
            for (int p2 = p1 + 1; p2 < numPlayers; p2++) {
                if (p1 != p2) {// avoid playing against yourself
                    for (int game = 0; game < numGames; game++) {
                        ...
                        if (verbose)
                            System.out.println("Result for Game " + game);
    ...
    /**
     * Tries to execute a Player class' method by using threads a layer of
     * protection in case
    ...
    /**
     * Initializes each player and each player profile with all the relevant
     * knowledge needed to start a game
    ...
    public static void initializePlayers(Graph g, PlayerProfile p1Profile, Player p1, PlayerProfile p2Profile,
        ...
        // initialize players
        tryPlayer(new PlayerDriver(PlayerState.INIT, p1)); // Try to initialize
                                                           // player 1
        tryPlayer(new PlayerDriver(PlayerState.INIT, p2)); // Try to initialize
                                                           // player 1
    }

    /**
     * Makes necessary changes to local version of the graph and
     * notifies/updates the current and opponent player
        ...
        if (isValidMove(graph, cpProfile.getCurrentLocation(), a.nodeID)) {
            switch (a.move) {
                case MOVE:
                    ...
                    tryPlayer(new PlayerDriver(PlayerState.OPP_RESULT, opponent, a.nodeID, false, null)); // Try
                                                                                                          // to
                                                                                                          // notify
                                                                                                          // opponent
                                                                                                          // of
                                                                                                          // result
                    tryPlayer(new PlayerDriver(PlayerState.RESULT, currentPlayer, a.nodeID, null)); // Try
                                                                                                    // to
                                                                                                    // notify
                                                                                                    // player
                                                                                                    // of
                    ...
                    if (verbose)
                        System.out.println(currentPlayer.getName() + " moved to node " + a.nodeID);
                    break;
                case PICKUP:
                    cpProfile.setCurrentLocation(a.nodeID);
                    currentPlayer.setCurrentNode(a.nodeID);
                ...
                default:
                    ...
                    if (verbose)
                        System.out.println(currentPlayer.getName() + " performed an invalid action default case hit");
                    break;
            }
        } else {
            tryPlayer(new PlayerDriver(PlayerState.OPP_RESULT, opponent, a.nodeID, false, null)); // Try
            ...
            tryPlayer(new PlayerDriver(PlayerState.RESULT, currentPlayer, cpProfile.getCurrentLocation(), null)); // Try
                                                                                                                  ...
                                                                                                                  // of
                                                                                                                  // result
            // opponent.opponentAction(cpProfile.getCurrentLocation(), false,
            // null);
            // currentPlayer.actionResult(cpProfile.getCurrentLocation(),null);
            if (verbose)
                System.out.println(currentPlayer.getName() + " performed an invalid action");
        }
    }
    ...
    /**
     * Should only be called by runMatches() as that method has the necessary
     * initializations to run this method
    ...
    private static void oneRound(PlayerProfile p1Profile, Player p1, PlayerProfile p2Profile, Player p2, int gameSeed) {
        ...
        for (int i = 0; i < Parameters.NUM_TURNS; i++) {
            // Checks if player 1 is finished, if not, has him/her make one
            // move, then registers the turn the gamemaster and both players
            if (p1Profile.getHandSize() < Parameters.MAX_HAND) {
                // oneTurn(p1);
                tryPlayer(new PlayerDriver(PlayerState.MAKE_ACTION, p1)); // Try
                                                                          // to
                                                                          // have
                                                                          // player
                                                                          // 1
            ...
            // Checks if player 2 is finished, if not, has him/her make one
            // move, then registers the turn the gamemaster and both players
            if (p2Profile.getHandSize() < Parameters.MAX_HAND) {
                // oneTurn(p2);
                tryPlayer(new PlayerDriver(PlayerState.MAKE_ACTION, p2)); // Try
                                                                          // to
                                                                          // have
                                                                          // player
                                                                          // 1
            ...
            if (verbose)
                System.out.println("Player 1's Hand: " + p1Profile.getCurrentHand());
            if (verbose)
                System.out.println("Player 2's Hand: " + p2Profile.getCurrentHand());
            if (verbose)
    ...
    /**
     * Runs a specified number of matches on the same graph against two players.
     * One match is
     * considered 2 rounds where the second round reverses the locations and
     * hands of the 2 players.
     * 
     * @param gameSeed number representing the seed used to generate the graph
     * @param p1Name The player that will act first in the first round of a
     *            match (will act as player 2 in the second round)
     * @param p2Name The player that will act second in the first round of a
     *            match (will act as player 1 in the first round)
     * @return the hands
     */
    public static Hand[] runMatches(int gameSeed, String p1Name, String p2Name) {
        Hand[] hands = new Hand[4];// 4 hands p1 & p2 vs p2 and p1
        if (getPlayer(p1Name) == null || getPlayer(p2Name) == null) {// make
                                                                     // sure
                                                                     // players
                                                                     // are
                                                                     // valid
        ...
        if (verbose)
            System.out.println("Game " + Integer.toString(gameSeed) + " starting...");
        ...
        /////////////////////////////////////////////////////////////////////////////
        // Exact same code as above, but reverses the roles of player 1 and
        ///////////////////////////////////////////////////////////////////////////// player
        ///////////////////////////////////////////////////////////////////////////// 2//
        /////////////////////////////////////////////////////////////////////////////
        // Game initialization
    ...
    private static void evaluateHands(Hand hand1, Hand hand2, float[] ranks, double[] wins, int p1, int p2,
        ...
        if (hand1.size() != Parameters.MAX_HAND && hand2.size() != Parameters.MAX_HAND) {
            if (verbose)
                System.out.println("No player was able to get a full hand.");
        } else if (hand1.size() != Parameters.MAX_HAND) {// p2 made it to 5 but
                                                         // p1 didn't
            wins[p2]++;
